...
async def test_create_agent_all_fields(client):
    pid = await _create_project(client)
    payload = {"name": "Analyst", "description": "Data bot", "system_prompt": "Be concise.", "top_k": 5}
...
async def test_get_agent(client):
    pid = await _create_project(client)
    aid = (await client.post(f"/api/projects/{pid}/agents", json={"name": "Finder"})).json()["id"]
...
async def test_update_agent_name(client):
    pid = await _create_project(client)
    aid = (await client.post(f"/api/projects/{pid}/agents", json={"name": "Old"})).json()["id"]
...
async def test_update_agent_top_k(client):
    pid = await _create_project(client)
    aid = (await client.post(f"/api/projects/{pid}/agents", json={"name": "Bot"})).json()["id"]
...
async def test_update_agent_not_found(client):
    r = await client.put("/api/agents/00000000-0000-0000-0000-000000000000", json={"name": "X"})
    assert r.status_code == 404
...
async def test_delete_agent(client):
    pid = await _create_project(client)
    aid = (await client.post(f"/api/projects/{pid}/agents", json={"name": "Del"})).json()["id"]
